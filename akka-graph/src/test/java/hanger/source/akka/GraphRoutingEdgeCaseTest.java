...
public class GraphRoutingEdgeCaseTest {
    ...
    void testLargeScaleGraph() {
        ...
        try {
            ...
            int N = 20;
            List<GraphCompiler.GraphNodeDef> nodes = new java.util.ArrayList<>();
            List<GraphCompiler.GraphEdgeDef> edges = new java.util.ArrayList<>();
            for (int i = 0; i < N; i++) {
                int idx = i;
                nodes.add(new GraphCompiler.GraphNodeDef("N" + idx,
                    (state, config) -> {
                ...
                if (i < N - 1) {
                    edges.add(new GraphCompiler.GraphEdgeDef("N" + idx, "", "N" + (idx + 1)));
                }
            }
            edges.add(new GraphCompiler.GraphEdgeDef("N" + (N - 1), "", "END"));
