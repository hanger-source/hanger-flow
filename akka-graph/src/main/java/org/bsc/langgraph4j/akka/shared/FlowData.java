package org.bsc.langgraph4j.akka.shared;
...
public abstract class FlowData<T> {
     ...
     */
    public T fragment() {
        if (isFragment()) return ((Fragment<T>) this).item;
     ...
     */
    public T result() {
        if (isDone()) return ((Done<T>) this).result;
    ...
    public Throwable error() {
        if (isError()) return ((Error<T>) this).error;
