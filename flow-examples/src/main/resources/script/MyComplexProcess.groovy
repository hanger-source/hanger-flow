...
flow {
    ...
    onEnter {
        log "[FLOW ON ENTER] 订单流程开始，订单ID: ${context.orderId ?: 'N/A'}。初始化状态。"
        context.status = "初始化"
        // 模拟用户设置（用于条件分支判断）
        context.user = [prefersEmailNotification: true, hasPhoneNumber: true]
    }

    onError {
        // 这里的 onError 捕获未在 Task 级别处理的、导致流程中断的错误
        log "[FLOW ON ERROR] 订单处理全局失败，订单ID: ${context.orderId ?: 'N/A'}。错误: ${exception.message}。即将跳转到流程错误处理。"
        context.errorMessage = "全局流程错误: ${exception.message}" // 传递错误信息
    } to '流程错误处理'
    ...
    task {
        ...
        run {
            println "任务 [订单初始化] 执行中，订单ID: ${context.orderId ?: 'N/A'}"
            context.status = "待处理"
            // 模拟获取订单商品列表，用于后续动态处理
            context.orderItems = [
    ...
    task {
        ...
        onEnter {
            log "[TASK ON ENTER] 开始库存检查，订单ID: ${context.orderId ?: 'N/A'}"
        }

        run {
            println "任务 [库存检查] 执行中，订单ID: ${context.orderId ?: 'N/A'}"
            // 模拟遍历订单商品并检查库存
            boolean allStockOk = true
            context.orderItems.each { item ->
                // 模拟根据商品ID或类型检查库存
                if (item.id == 'item001' && item.qty > 0) { // 假设item001库存不足模拟
                    allStockOk = false
                    println "商品 ${item.id} 库存不足！"
                } else {
                    println "商品 ${item.id} 库存充足。"
                }
            }
            context.stockOk = allStockOk
        }

        // 根据库存检查结果进行条件跳转
        next { context.stockOk } to "支付处理"
        next { !context.stockOk } to "通知库存不足" // 库存不足直接通知并结束订单

        // Task 级别的错误处理，如果此任务失败（例如调用库存服务超时），会跳转到指定的错误处理任务
        onError {
            log "[ERROR] 库存检查任务失败: ${exception.message}"
            context.errorMessage = "库存检查异常: ${exception.message}"
    ...
    task {
        ...
        run {
            println "任务 [支付处理] 执行中，订单ID: ${context.orderId ?: 'N/A'}"
            // 模拟支付成功或失败
            // context.paid = true // 模拟成功
            context.paid = false // 模拟失败
            println "支付结果: ${context.paid ? '成功' : '失败'}"
        }
        // 根据支付结果进行条件跳转
        next { context.paid } to "支付成功并行处理" // 支付成功后，进入并行处理的起点
        next { !context.paid } to "通知支付失败" // 支付失败，通知用户并结束
    }
    ...
    task {
        ...
        run {
            println "任务 [通知库存不足] 执行中，订单ID: ${context.orderId ?: 'N/A'}"
            // 模拟调用 NotificationService.sendSms(context.userPhone, "很抱歉，您的订单商品库存不足。")
            println "短信通知用户：库存不足。"
            context.status = "库存不足，已关闭"
        }
        nextTo END // 此分支流程结束
    }
    ...
    task {
        ...
        run {
            println "任务 [通知支付失败] 执行中，订单ID: ${context.orderId ?: 'N/A'}"
            println "邮件通知用户：支付失败，请尝试重新支付或联系客服。"
            context.status = "支付失败，待重试"
        }
        nextTo END // 此分支流程结束
    }
    ...
    parallel {
        ...
        branch '发送订单确认邮件' when { context.user.prefersEmailNotification } // 如果用户偏好邮件，则发送确认邮件
    ...
    task {
        ...
        run {
            println "并行分支 [物流分配]：任务 [物流分配] 执行中...分配单号: L${context.orderId}"
            context.logisticsAssigned = true
        }
    }
    ...
    task {
        ...
        run {
            println "并行分支 [拣货打包]：任务 [拣货打包] 执行中..."
            context.itemsPacked = true
        }
    }
    ...
    task {
        ...
        run {
            println "条件并行分支 [发送订单确认邮件]：任务 [发送订单确认邮件] 执行中..."
    ...
    task {
        ...
        run {
            println "异步通知流 [发送短信通知]：任务 [发送短信通知] 执行中..."
    ...
    task {
        ...
        run {
            println "任务 [订单完成] 执行中，订单ID: ${context.orderId ?: 'N/A'}"
            context.status = "已完成"
            println "流程成功结束！"
        }
    ...
    task {
        ...
        run {
            println "[ERROR GLOBAL] 捕获到流程级别错误，执行通用错误处理。错误信息: ${context.errorMessage ?: '未知错误'}"
            // 这里可以进行：回滚操作、告警通知、记录详细日志等
            context.status = "流程异常终止"
        }
    ...
    task {
        ...
        run {
            println "[ERROR TASK] 捕获到任务级别错误，日志记录中。错误信息: ${context.errorMessage ?: '未知错误'}"
            // 记录到具体的错误日志系统
            context.status = "任务失败"
        }
    ...
    task {
        ...
        run {
            println "[ERROR NOTIFICATION] 通知管理员订单处理失败，订单ID: ${context.orderId ?: 'N/A'}。"
            // 模拟发送邮件或触发告警系统
        }
    }
}