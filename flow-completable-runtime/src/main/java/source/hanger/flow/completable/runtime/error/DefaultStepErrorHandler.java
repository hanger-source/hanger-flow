...
public class DefaultStepErrorHandler implements StepErrorHandler {
    ...
    public FlowResult handleError(StepDefinition step, FlowExecutionContext context, Exception error) {
        ...
        FlowLogger.log(FlowLogger.Level.DEBUG, new FlowLogContext("ERROR_HANDLER", null, executionId, stepName),
            "步骤错误处理: " + error.getMessage());
    ...
    private FlowResult handleTaskStepError(TaskStepDefinition taskStep, FlowExecutionContext context, Exception error) {
        FlowLogger.log(FlowLogger.Level.DEBUG,
            new FlowLogContext("ERROR_HANDLER", null, context.getExecutionId(), taskStep.getName()),
            "任务步骤错误处理");
        // 可以在这里添加任务特定的错误处理逻辑
    ...
    private FlowResult handleGenericStepError(StepDefinition step, FlowExecutionContext context, Exception error) {
        FlowLogger.log(FlowLogger.Level.DEBUG,
            new FlowLogContext("ERROR_HANDLER", null, context.getExecutionId(), step.getName()), "通用步骤错误处理");
        return new FlowResult(context.getExecutionId(), FlowStatus.ERROR, context.getInputs(), error);
    }
} 